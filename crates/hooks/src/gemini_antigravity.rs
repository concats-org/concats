...
const AGENT: &str = "gemini-antigravity";
const MATCHER: &str = "write_file|replace";

pub(crate) struct GeminiAntigravityAgent;

impl crate::Agent for GeminiAntigravityAgent {
    fn name(&self) -> &'static str {
        AGENT
    }

    fn is_detected(&self) -> bool {
        dirs::home_dir().is_some_and(|home| home.join(".gemini").join("antigravity").is_dir())
    }
        ...
        crate::dispatch_simple(
            ...
            |event| match event {
                ...
                _ => Err(Error::session(format!(
                    "unknown Gemini Antigravity hook event: {event}"
                ))),
            },
        )
    }
...
fn config_path() -> Result<PathBuf> {
    dirs::home_dir()
        .map(|h| h.join(".gemini").join("antigravity").join("settings.json"))
...
fn install_hooks(mut value: serde_json::Value, binary: &Path) -> Result<serde_json::Value> {
    let root = value
        .as_object_mut()
        .ok_or_else(|| Error::session("gemini-antigravity config root is not an object"))?;

    let tools = root
        .entry("tools")
        .or_insert_with(|| serde_json::json!({}))
        .as_object_mut()
        .ok_or_else(|| Error::session("gemini-antigravity tools is not an object"))?;
    ...
    let hooks = root
        ...
        .or_insert_with(|| serde_json::json!({}))
        .as_object_mut()
        .ok_or_else(|| Error::session("gemini-antigravity hooks is not an object"))?;
    ...
    for event in ["BeforeTool", "AfterTool"] {
        let entries = hooks
            ...
            .as_array_mut()
            .ok_or_else(|| {
                Error::session(format!("gemini-antigravity hooks.{event} is not an array"))
            })?;
...
mod tests {
    ...
    mod install_hooks {
        ...
        fn preserves_other_hooks() {
            let config = serde_json::json!({
                "hooks": {
                    "AfterTool": [
                        ...
                        { "matcher": "write_file|replace", "hooks": [{ "type": "command", "command": "concats hook gemini-antigravity AfterTool" }] }
                    ]
                }
            });
            ...
            assert!(data.contains("concats hook gemini-antigravity AfterTool"));
            assert!(data.contains("concats hook gemini-antigravity BeforeTool"));
    ...
    mod remove_hooks {
        ...
        fn removes_only_concats_hooks() {
            ...
            let data = serde_json::to_string(&config).unwrap();
            assert!(data.contains("other-tool hook"));
            assert!(!data.contains("concats hook gemini-antigravity"));
        }
    }
}
